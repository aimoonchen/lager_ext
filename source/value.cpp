    ...
    std::visit(
        [&](auto&& arg) {
            ...
            if constexpr (std::is_same_v<T, std::string>) {
                ...
                std::cout << std::string(depth * 2, ' ') << prefix << format_float_array(arg, "vec2") << "\n";
            } else if constexpr (std::is_same_v<T, Vec3>) {
...
public:
    ...
    bool has_bytes(std::size_t n) const { return pos + n <= size; }
...
private:
    ...
    char consume() { return pos_ < json_.size() ? json_[pos_++] : '\0'; }
